...
const ErrorCodes = require('./const');
const db = require('./db');
...
class User {
    ...
    async createUser(data) {
        ...
        try {
            ...
            if (result.modifiedCount || result.upsertedId === null) {
                return new Error('User already exists', ErrorCodes.UserExists);
            }
    ...
    isValidUserData(data) {
        if (typeof data !== 'object') {
            return new Error('Invalid User data', ErrorCodes.InvalidUser);
        } else if (typeof data.password !== 'string' || data.password.length < config.PASSWORD_LENGTH) {
            return new Error('Invalid password. Minimum length ' + config.PASSWORD_LENGTH, ErrorCodes.InvalidPassword);
